...
def sudga_topshirilganlar_excel_eksport(parent):
    """
    Sudga topshirilgan barcha ishlarni Excel faylga eksport qiladi —
    Chora ko'rish va Davo Ariza bo'limlarining ikkalasida ham
    ishlatiladigan umumiy funksiya.
    """
    royxat = db.get_sudga_topshirilganlar_royxati()
    if not royxat:
        messagebox.showinfo("Diqqat", "Sudga topshirilgan hech qanday ish topilmadi.")
        return
    ...
    for item in royxat:
        ...
        hujjat_id = util.mijoz_hujjat_id(prow, turi) if prow else ''
        asosiy = xat.get('davo_summasi_asosiy') or 0
        foiz = xat.get('davo_summasi_foiz') or 0
        jarima = xat.get('davo_summasi_jarima') or 0
        rows.append([
            xat['anketa_raqami'], hujjat_id, xat['mijoz_nomi'], turi,
            xat.get('sud_ish_raqami', '') or '', xat.get('sud_topshirilgan_sana', '') or '',
            asosiy, foiz, jarima, asosiy + foiz + jarima,
            "✓ MIBga o'tkazilgan" if xat.get('mib_holati') == 'otkazildi' else "Kutilmoqda",
        ])
    headers = ['Anketa raqami', 'PINFL/STIR', 'Mijoz', 'Turi', 'Sud ish raqami',
               'Sudga topshirilgan sana', 'Asosiy qarz', 'Foiz', 'Penya',
               'Jami sud summasi', 'MIB holati']
...
def bajar_ommaviy_davo_ariza(parent, app, nomzodlar):
    """
    Bir nechta mijoz uchun BIRDAN Davo ariza tayyorlaydi — har biriga
    o'zining kafil/garov ma'lumotiga (taminot) qarab avtomatik to'g'ri
    turi tanlanadi (xuddi Davo Ariza bo'limidagi "Tanlanganlar uchun
    Davo ariza tayyorlash" kabi).

    nomzodlar: har biri {'portfel': ..., 'xat': ...} dict ro'yxati —
    xat mavjud bo'lishi va hali Davo arizasi yo'qligi talab qilinadi
    (chaqiruvchi tomonda tekshiriladi).

    MUHIM: bu faqat Davo arizani TAYYORLAYDI — "Olib kelindi" deb
    TASDIQLASH (SSPdan qaytgandan keyin) hamon har bir mijoz uchun
    ALOHIDA (bittadan) qilinishi kerak, chunki bu bosqichda har biriga
    individual hujjat (SSP tasdiqlagan skan, ish raqami) talab qilinadi.
    """
    if not nomzodlar:
        messagebox.showinfo(
            "Diqqat",
            "Tanlangan mijozlar orasida 'Davo ariza tayyorlash' bosqichidagi hech kim yo'q."
        )
        return 0

    javob = messagebox.askyesno(
        "Tasdiqlash",
        f"{len(nomzodlar)} ta mijoz uchun Davo ariza tayyorlanadi — har biriga o'zining "
        f"kafil/garov ma'lumotiga qarab avtomatik to'g'ri turi tanlanadi.\n\n"
        f"Diqqat: 'Olib kelindi' deb tasdiqlash (SSPdan qaytgandan keyin) hamon har bir "
        f"mijoz uchun ALOHIDA (bittadan) qilinishi kerak bo'ladi.\n\nDavom etaymi?"
    )
    if not javob:
        return 0
    ...
    yaratildi, otkazib_yuborildi = 0, 0
    turlar_soni = {}
    xatolar = []
    ...
    msg = f"{yaratildi} ta Davo ariza tayyorlandi."
    if turlar_soni:
    ...
    if otkazib_yuborildi:
        msg += f"\n\nℹ {otkazib_yuborildi} ta anketa uchun Davo ariza allaqachon mavjud edi."
    if xatolar:
        msg += f"\n\n{len(xatolar)} ta xatoda:\n" + '\n'.join(xatolar[:5])
    messagebox.showinfo("Bajarildi", msg)
    return yaratildi
...
class TaminotDialog(tk.Toplevel):
    ...
    def save(self):
        ...
        # Xavfsizlik: agar kafil/garov ma'lumoti to'ldirilgan bo'lsa-yu,
        # foydalanuvchi "Ta'minot turi" dropdown'ini "yo'q"da unutib qoldirsa
        # (Davo ariza turini noto'g'ri tanlashiga sabab bo'ladi) — avtomatik
        # to'g'irlaymiz.
        kafil_bor = bool(fields.get('kafil_ism', ''))
        garov_bor = bool(fields.get('garov_tavsifi', ''))
        if fields.get('taminot_turi') == 'yoq' and (kafil_bor or garov_bor):
            if kafil_bor and garov_bor:
                fields['taminot_turi'] = 'kafillik_garov'
            elif kafil_bor:
                fields['taminot_turi'] = 'kafillik'
            else:
                fields['taminot_turi'] = 'garov'
...
class ReestrSanaDialog(tk.Toplevel):
    """SSPga yuboriladigan Reestr xati uchun chiquvchi xat raqami va sanasini so'raydi."""
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Reestr xati ma'lumotlari")
        self.geometry("400x220")
        self.configure(bg=BG)
        self.result = None

        ttk.Label(self, text="SSPga yuboriladigan Reestr xati",
                  style='Header.TLabel').pack(anchor='w', padx=16, pady=(16, 10))

        row1 = ttk.Frame(self)
        row1.pack(fill='x', padx=16, pady=6)
        ttk.Label(row1, text="Chiquvchi xat raqami:", width=18).pack(side='left')
        self.raqam_var = tk.StringVar()
        ttk.Entry(row1, textvariable=self.raqam_var, width=20).pack(side='left')
        ttk.Label(self, text="(masalan: 03/999)", style='Sub.TLabel').pack(anchor='w', padx=16)

        row2 = ttk.Frame(self)
        row2.pack(fill='x', padx=16, pady=6)
        ttk.Label(row2, text="Sana:", width=18).pack(side='left')
        self.sana_var = tk.StringVar(value=datetime.date.today().strftime('%d.%m.%Y'))
        ttk.Entry(row2, textvariable=self.sana_var, width=20).pack(side='left')

        btns = ttk.Frame(self)
        btns.pack(fill='x', padx=16, pady=20, side='bottom')
        ttk.Button(btns, text="Bekor qilish", command=self.destroy).pack(side='right', padx=4)
        ttk.Button(btns, text="✓ Tayyorlash", style='Accent.TButton', command=self.confirm).pack(side='right', padx=4)

        self.transient(parent)
        self.grab_set()

    def confirm(self):
        raqam = self.raqam_var.get().strip()
        sana = self.sana_var.get().strip()
        if not raqam or not sana:
            messagebox.showerror("Xato", "Xat raqami va sanani kiriting.")
            return
        self.result = (raqam, sana)
        self.destroy()
...
class SudBazaTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        toolbar_excel = ttk.Frame(self)
        toolbar_excel.pack(fill='x', padx=20, pady=(8, 0))
        ttk.Button(toolbar_excel, text="📤 Sudga topshirish shablonini yuklab olish",
                   command=self.export_topshirish_shablon).pack(side='left')
        ttk.Button(toolbar_excel, text="📥 To'ldirilgan Excel'ni yuklash (ommaviy belgilash)",
                   style='Accent.TButton', command=self.import_topshirish_excel).pack(side='left', padx=8)
        ttk.Label(toolbar_excel, text="(shablonda 'Sud ish raqami' va 'Sanasi' to'ldirilgan "
                                       "qatorlar avtomatik 'Sudga topshirildi' deb belgilanadi)",
                  style='Sub.TLabel', wraplength=500, justify='left').pack(side='left', padx=8)
    ...
    def export_topshirish_shablon(self):
        """
        Hozirgi ro'yxatdagi (Palatadan/SSPdan olib kelingan, hali sudga
        topshirilmagan) mijozlarni Excel shabloniga chiqaradi — bo'sh
        "Sud ish raqami" va "Sanasi" ustunlarini to'ldirib qaytarish uchun.
        """
        if not self._rows_cache:
            messagebox.showinfo("Diqqat", "Ro'yxat bo'sh — eksport qilinadigan mijoz yo'q.")
            return
        out_path = filedialog.asksaveasfilename(
            title="Excel shablonni saqlash", defaultextension=".xlsx",
            initialfile="sudga_topshirish_shabloni.xlsx", filetypes=[("Excel", "*.xlsx")]
        )
        if not out_path:
            return
        import pandas as pd
        ...
        for item in self._rows_cache.values():
            x = item['xat']
            rows.append([x['anketa_raqami'], x['mijoz_nomi'], item['sud_nomi'], item['jami'], '', ''])
        headers = ['Anketa raqami', 'Mijoz', 'Sud turi', 'Jami qarzdorlik',
                   'Sud ish raqami', "Sanasi (kun.oy.yil, masalan 20.08.2026)"]
        df = pd.DataFrame(rows, columns=headers)
        df.to_excel(out_path, index=False)
        messagebox.showinfo(
            "Tayyor",
            f"{len(rows)} ta mijoz shablonga chiqarildi:\n{out_path}\n\n"
            "'Sud ish raqami' va 'Sanasi' ustunlarini to'ldirib, so'ng "
            "'To'ldirilgan Excel'ni yuklash' orqali qaytaring — faqat "
            "IKKALASI HAM to'ldirilgan qatorlar 'Sudga topshirildi' deb belgilanadi."
        )
        webbrowser.open(out_path)

    def import_topshirish_excel(self):
        """
        To'ldirilgan shablonni o'qib, 'Sud ish raqami' VA 'Sanasi' ikkalasi
        ham to'ldirilgan qatorlarni avtomatik 'Sudga topshirildi' deb
        belgilaydi — bir nechta mijozni bir vaqtda, qo'lda bittalab
        kiritmasdan.
        """
        filepath = filedialog.askopenfilename(
            title="To'ldirilgan Excel faylni tanlang", filetypes=[("Excel", "*.xlsx *.xls")]
        )
        if not filepath:
            return
        import pandas as pd
        try:
            ...
            messagebox.showerror("Xato", f"Faylni o'qib bo'lmadi: {e}")
            return
        ...
        if 'Anketa raqami' not in df.columns or 'Sud ish raqami' not in df.columns or not sana_col:
            messagebox.showerror(
                "Xato",
                "Excel faylida kerakli ustunlar topilmadi. Iltimos, dastur bergan asl "
                "shablon tuzilmasini o'zgartirmang."
            )
            return
        ...
        for _, r in df.iterrows():
            ...
            xat_id = None
            for item in self._rows_cache.values():
                if item['xat']['anketa_raqami'] == anketa:
                    xat_id = item['xat']['id']
                    break
            if not xat_id:
                conn = db.get_conn()
                row = conn.execute(
                    ...
                    "AND (sud_holati IS NULL OR sud_holati != 'topshirildi')", (anketa,)
                ).fetchone()
                conn.close()
                if row:
                    xat_id = row['id']
            ...
            db.mark_sud_topshirildi(xat_id, str(ish_raqami).strip(), sana_str)
            yangilandi += 1

        msg = f"{yangilandi} ta mijoz 'Sudga topshirildi' deb belgilandi."
        if otkazib_yuborildi:
            msg += f"\n\nℹ {otkazib_yuborildi} ta qatorda 'Sud ish raqami' yoki 'Sanasi' " \
                   f"to'ldirilmagani uchun o'tkazib yuborildi."
        if topilmadi:
            msg += f"\n\n⚠ {topilmadi} ta anketa ro'yxatda (yoki 'olib kelindi, sudga " \
                   f"topshirilmagan' holatda) topilmadi."
        messagebox.showinfo("Bajarildi", msg)
        self.refresh()
        self.app.tab_dashboard.refresh()
...
class DavoArizaTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        ttk.Button(gen_frame, text="⚖ Sudga topshirilganlar (Excel)",
                   command=lambda: sudga_topshirilganlar_excel_eksport(self)).pack(side='left', padx=8)
        ...
        paket_frame = ttk.Frame(self)
        paket_frame.pack(fill='x', padx=20, pady=(6, 0))
        ttk.Label(paket_frame, text="Paket hajmi:").pack(side='left')
        self.paket_var = tk.StringVar(value='30')
        ttk.Combobox(paket_frame, textvariable=self.paket_var, values=['10', '30', '50', '100', 'Barchasi'],
                     state='readonly', width=10).pack(side='left', padx=8)
        ttk.Button(paket_frame, text="① Birinchi paketni belgilash",
                   command=self.select_paket).pack(side='left', padx=6)

        ariza_filter_frame = ttk.Frame(self)
        ariza_filter_frame.pack(fill='x', padx=20, pady=(8, 0))
        ttk.Label(ariza_filter_frame, text="Holat bo'yicha filtrlash:").pack(side='left')
        self.holat_filter_var = tk.StringVar(value="Barchasi")
        ttk.Combobox(ariza_filter_frame, textvariable=self.holat_filter_var,
                     values=["Barchasi", "Davo ariza tayyorlangan (barchasi)", "Tayyor (kutilmoqda)",
                             "Olib kelindi", "Muddati o'tgan", "Hali tayyorlanmagan"],
                     state='readonly', width=30).pack(side='left', padx=8)

        ttk.Label(ariza_filter_frame, text="   Turi:").pack(side='left', padx=(16, 0))
        self.turi_filter_var = tk.StringVar(value="Barchasi")
        ttk.Combobox(ariza_filter_frame, textvariable=self.turi_filter_var,
                     values=["Barchasi", "Jismoniy", "Yuridik", "YaTT"],
                     state='readonly', width=12).pack(side='left', padx=8)

        ttk.Button(ariza_filter_frame, text="Filtrlash", command=self.refresh).pack(side='left', padx=8)
        ttk.Button(ariza_filter_frame, text="🗑 Tayyorlangan arizani o'chirish (belgilanganlar)",
                   command=self.davo_ariza_ochirish).pack(side='left', padx=(16, 0))

        ariza_turi_frame = ttk.Frame(self)
        ariza_turi_frame.pack(fill='x', padx=20, pady=(8, 0))
        ttk.Label(ariza_turi_frame, text="Tayyorlangan ariza turi bo'yicha filtrlash:").pack(side='left')
        self.tayyor_turi_filter_var = tk.StringVar(value="Barchasi")
        tayyor_turi_values = ["Barchasi"] + list(letters.DAVO_ARIZA_NOMLARI.values())
        ttk.Combobox(ariza_turi_frame, textvariable=self.tayyor_turi_filter_var,
                     values=tayyor_turi_values, state='readonly', width=42).pack(side='left', padx=8)
        ttk.Button(ariza_turi_frame, text="Filtrlash", command=self.refresh).pack(side='left', padx=8)
        ttk.Button(ariza_turi_frame, text="📎 Belgilanganlarni bitta faylga birlashtirish",
                   style='Accent.TButton', command=self.arizalarni_birlashtir).pack(side='left', padx=(16, 0))

        hisoblagich_frame = ttk.Frame(self)
        hisoblagich_frame.pack(fill='x', padx=20, pady=(8, 0))
        self.belgilangan_soni_var = tk.StringVar(value="Belgilangan: 0 ta")
        ttk.Label(hisoblagich_frame, textvariable=self.belgilangan_soni_var,
                  font=('Segoe UI', 10, 'bold'), foreground=NAVY_DARK).pack(side='left')

        toolbar_reestr = ttk.Frame(self)
        toolbar_reestr.pack(fill='x', padx=20, pady=(8, 8))
        ttk.Button(toolbar_reestr, text="📋 SSPga Reestr tayyorlash (belgilanganlar)",
                   style='Accent.TButton', command=self.reestr_tayyorlash).pack(side='left')
        ttk.Label(toolbar_reestr, text="(faqat Davo arizasi tayyorlangan mijozlar reestrga kiradi)",
    ...
    def _qayta_chiz_belgi(self, iid):
        ...
        vals[0] = belgi
        self.tree.item(iid, values=vals)
        self.belgilangan_soni_var.set(f"Belgilangan: {len(self.checked)} ta")
    ...
    def select_paket(self):
        """Ro'yxatdagi birinchi N ta qatorni belgilaydi — katta ro'yxatni
        bosqichma-bosqich (paket-paket) ishlash uchun."""
        paket = self.paket_var.get()
        all_items = self.tree.get_children()
        if not all_items:
            messagebox.showinfo("Diqqat", "Ro'yxat bo'sh.")
        ...
        if to_select:
            self.tree.see(to_select[0])
        messagebox.showinfo("Tanlandi", f"{len(to_select)} ta mijoz belgilandi (paket hajmi: {paket}).")

    def davo_ariza_ochirish(self):
        """
        Belgilangan mijozlarning TAYYORLANGAN Davo arizasini bazadan
        o'chiradi (fayl diskda qoladi, faqat baza yozuvi tozalanadi) —
        shunda ular qaytadan "Davo ariza tayyorlash kerak" bosqichiga
        qaytib, yangi Davo ariza yaratish mumkin bo'ladi ("allaqachon
        mavjud" xatosi bermay).

        Sudga topshirilgan ('topshirildi') ishlarga TEGILMAYDI — bunday
        holatlarni avval Sud bo'limida qayta ko'rib chiqish kerak.
        """
        ...
        bor = [item for item in selected if item['xat'].get('davo_ariza_fayl_yoli')]
        sudga_ketgan = [item for item in bor if item['xat'].get('sud_holati') == 'topshirildi']
        ochiriladigan = [item for item in bor if item['xat'].get('sud_holati') != 'topshirildi']

        if not ochiriladigan:
            messagebox.showinfo(
                "Diqqat",
                "Belgilangan mijozlar orasida o'chirish mumkin bo'lgan (tayyorlangan, lekin "
                "hali sudga topshirilmagan) Davo ariza topilmadi."
            )
            return

        matn = (f"{len(ochiriladigan)} ta mijozning tayyorlangan Davo arizasi bazadan "
                f"o'chiriladi (fayl diskda qoladi). Shundan keyin ular uchun YANGI Davo "
                f"ariza yaratishingiz mumkin bo'ladi.")
        if sudga_ketgan:
            matn += (f"\n\nℹ {len(sudga_ketgan)} ta mijoz sudga allaqachon topshirilgani "
                     f"uchun o'chirilmaydi.")
        matn += "\n\nDavom etaymi?"
        if not messagebox.askyesno("Tasdiqlash", matn):
            return

        for item in ochiriladigan:
            db.reset_davo_ariza(item['xat']['id'])
        messagebox.showinfo("Bajarildi", f"{len(ochiriladigan)} ta Davo ariza yozuvi tozalandi.")
        self.refresh()

    def arizalarni_birlashtir(self):
        """
        Belgilangan mijozlarning TAYYORLANGAN Davo arizalarini bitta
        umumiy Word (yoki PDF) faylga jamlaydi — chop etish/topshirish
        uchun qulay bo'lishi uchun.
        """
        selected = self._selected()
        nomzodlar = [item for item in selected if item['xat'].get('davo_ariza_fayl_yoli') and
                     os.path.exists(item['xat']['davo_ariza_fayl_yoli'])]
        otkazib_yuborildi = len(selected) - len(nomzodlar)

        if not nomzodlar:
            messagebox.showinfo(
                "Diqqat",
                "Belgilangan mijozlar orasida (diskda mavjud) tayyorlangan Davo ariza fayli "
                "topilmadi."
            )
            return
        if len(nomzodlar) < 2:
            messagebox.showinfo(
                "Diqqat",
                "Birlashtirish uchun kamida 2 ta tayyorlangan Davo ariza belgilanishi kerak."
            )
            return

        # Faqat .docx fayllar birlashtirilishi mumkin — agar PDF holida
        # saqlangan (masalan avvalroq PDF formatida tayyorlangan) bo'lsa,
        # bunday fayllarni birlashtirib bo'lmaydi, o'tkazib yuboriladi.
        docx_fayllar = [item['xat']['davo_ariza_fayl_yoli'] for item in nomzodlar
                         if item['xat']['davo_ariza_fayl_yoli'].lower().endswith('.docx')]
        pdf_bolganlar = len(nomzodlar) - len(docx_fayllar)

        if len(docx_fayllar) < 2:
            messagebox.showinfo(
                "Diqqat",
                "Birlashtirish faqat Word (.docx) formatidagi fayllar uchun ishlaydi. "
                "Belgilanganlar orasida yetarlicha .docx fayl topilmadi (PDF holida "
                "tayyorlangan fayllarni birlashtirib bo'lmaydi)."
            )
            return

        format_tanlov = soralsin_format(self, "Birlashgan fayl formati")
        if not format_tanlov:
            return

        out_path_docx = filedialog.asksaveasfilename(
            title="Birlashgan faylni saqlash", defaultextension=".docx",
            initialfile="Birlashgan_Davo_arizalar.docx", filetypes=[("Word", "*.docx")]
        )
        if not out_path_docx:
            return

        try:
            letters.birlashtir_docx(docx_fayllar, out_path_docx)
            natija_yoli = out_path_docx
            if format_tanlov.startswith('PDF'):
                try:
                    natija_yoli = letters.convert_docx_to_pdf(out_path_docx, delete_docx=True)
                except Exception as e:
                    messagebox.showwarning(
                        "Diqqat",
                        f"PDF'ga aylantirishda muammo yuz berdi ({e}) — fayl Word (.docx) "
                        f"holida saqlandi."
                    )
        except Exception as e:
            messagebox.showerror("Xato", f"Birlashtirishda xato: {e}")
            return

        msg = f"{len(docx_fayllar)} ta Davo ariza bitta faylga birlashtirildi:\n{natija_yoli}"
        if pdf_bolganlar:
            msg += f"\n\nℹ {pdf_bolganlar} ta belgilangan mijozning fayli PDF holida " \
                   f"bo'lgani uchun birlashtirilmadi."
        if otkazib_yuborildi:
            msg += f"\n\nℹ {otkazib_yuborildi} ta belgilangan mijozda tayyorlangan fayl " \
                   f"topilmadi."
        messagebox.showinfo("Bajarildi", msg)
        webbrowser.open(natija_yoli)
    ...
    def refresh(self):
        ...
        self._rows_cache = {}
        self.checked = set()
        self.belgilangan_soni_var.set("Belgilangan: 0 ta")
        ...
        for x in xatlar:
            ...
            turi_filtr = self.turi_filter_var.get()
            turi_map = {"Jismoniy": "jismoniy", "Yuridik": "yuridik", "YaTT": "yatt"}
            if turi_filtr != "Barchasi" and x['mijoz_turi'] != turi_map.get(turi_filtr):
                continue
            ...
            holat_filtr = self.holat_filter_var.get()
            if holat_filtr == "Davo ariza tayyorlangan (barchasi)" and not has_davo:
                continue
            if holat_filtr == "Hali tayyorlanmagan" and has_davo:
                continue
            if holat_filtr == "Tayyor (kutilmoqda)" and not (has_davo and not olib_kelindi and tag != 'otgan'):
                continue
            if holat_filtr == "Olib kelindi" and not olib_kelindi:
                continue
            if holat_filtr == "Muddati o'tgan" and tag != 'otgan':
                continue

            tayyor_turi_filtr = self.tayyor_turi_filter_var.get()
            if tayyor_turi_filtr != "Barchasi":
                haqiqiy_turi_kaliti = x.get('davo_ariza_turi')
                haqiqiy_turi_nomi = letters.DAVO_ARIZA_NOMLARI.get(haqiqiy_turi_kaliti, '')
                if haqiqiy_turi_nomi != tayyor_turi_filtr:
                    continue
    ...
    def reestr_tayyorlash(self):
        """
        Belgilangan (yoki tanlangan) mijozlar orasidan — faqat Davo arizasi
        TAYYORLANGANLARINI — SSPga (Savdo-Sanoat Palatasiga) yuboriladigan
        rasmiy Reestr xatiga jamlab, bitta hujjat qilib tayyorlaydi.
        """
        selected = self._selected()
        nomzodlar = [item for item in selected if item['xat'].get('davo_ariza_fayl_yoli')]
        otkazib_yuborildi = len(selected) - len(nomzodlar)

        if not nomzodlar:
            messagebox.showinfo(
                "Diqqat",
                "Belgilangan mijozlar orasida Davo arizasi tayyorlangan hech kim yo'q.\n\n"
                "Reestrga faqat 'Davo ariza tayyorlangan' mijozlar kiritiladi."
            )
            return

        dlg = ReestrSanaDialog(self)
        self.wait_window(dlg)
        if not dlg.result:
            return
        xat_raqami, xat_sanasi = dlg.result
        ...
        for item in nomzodlar:
            ...
            mijozlar_royxati.append({
                ...
                'mijoz_ism': xat['mijoz_nomi'],
                'summa': davo_summasi,
            })

        settings = db.get_all_settings()
        fname = f"Reestr_SSP_{letters.safe_filename(xat_sanasi)}.docx"
        out_path = os.path.join(bugungi_papka('Davo ariza'), fname)
        if not os.path.exists(letters.REESTR_SSP_TEMPLATE_PATH):
            messagebox.showerror(
                "Shablon topilmadi",
                "Reestr shabloni (reestr_ssp_shablon.docx) topilmadi.\n\n"
                "Bu — dastur .exe qilib qayta yig'ilganda 'templates' papkasi "
                "eng so'nggi (to'liq) versiya bilan almashtirilmagani sababli "
                "yuz berishi mumkin.\n\n"
                "Yechim: dastur kodi (qarz_nazorat papkasi) ni eng so'nggi "
                "arxivdan qayta oching — FAQAT 'templates' papkasini almashtirsangiz "
                "ham yetarli (qarz_nazorat.db faylingizga tegmang), so'ng "
                "build_exe.bat ni qayta ishga tushiring."
            )
            return
        try:
            letters.generate_reestr_ssp(out_path, mijozlar_royxati, xat_raqami, xat_sanasi, settings)
        except Exception as e:
            messagebox.showerror("Xato", str(e))
            return

        msg = f"{len(mijozlar_royxati)} ta mijoz uchun Reestr tayyorlandi:\n{out_path}"
        if otkazib_yuborildi:
            msg += f"\n\nℹ {otkazib_yuborildi} ta belgilangan mijoz Davo arizasi hali " \
                   f"tayyorlanmagani uchun reestrga kiritilmadi."
        messagebox.showinfo("Tayyor", msg)
        webbrowser.open(out_path)
...
class Nazorat95413Tab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        ttk.Label(filter_frame, text="   Turi:").pack(side='left', padx=(16, 0))
        self.turi_filter_var = tk.StringVar(value="Barchasi")
        ttk.Combobox(filter_frame, textvariable=self.turi_filter_var,
                     values=["Barchasi", "Jismoniy", "Yuridik", "YaTT"],
                     state='readonly', width=12).pack(side='left', padx=8)
    ...
    def refresh_table(self):
        ...
        for r in self._royxat_cache:
            ...
            if turi_filtr != "Barchasi" and turi != turi_map.get(turi_filtr):
                continue
...
class ChoraKorishTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        ttk.Button(toolbar, text="⚖ Sudga topshirilganlar (Excel)",
                   command=lambda: sudga_topshirilganlar_excel_eksport(self)).pack(side='left', padx=8)
        ...
        paket_frame = ttk.Frame(self)
        paket_frame.pack(fill='x', padx=20, pady=(8, 0))
        ttk.Label(paket_frame, text="Paket hajmi:").pack(side='left')
        self.paket_var = tk.StringVar(value='30')
        ttk.Combobox(paket_frame, textvariable=self.paket_var, values=['10', '30', '50', '100', 'Barchasi'],
                     state='readonly', width=10).pack(side='left', padx=8)
    ...
    def select_paket(self):
        """Ro'yxatdagi (joriy filtr bo'yicha ko'rinib turgan) birinchi N ta
        qatorni belgilaydi — katta ro'yxatni bosqichma-bosqich (paket-paket)
        ishlash uchun."""
        paket = self.paket_var.get()
        all_items = self.tree.get_children()
        if not all_items:
    ...
    def refresh_table(self):
        ...
        for r in self._royxat_cache:
            if filtr != "Barchasi" and r['chora_nomi'] != filtr:
                continue
            if turi_filtr != "Barchasi" and r['turi'] != turi_map.get(turi_filtr):
                continue
    ...
    def bajar_amal(self):
        ...
        Bir nechta mijoz BIRGA tanlansa — xat yuborish VA Davo ariza
        tayyorlash choralari uchun ommaviy (paketlab) ishlaydi (har biriga
        o'zining kafil/garov ma'lumotiga qarab). MIB harakati — individual
        hujjat kerak bo'lgani uchun faqat bittadan bajariladi. "Olib
        kelindi" deb tasdiqlash ham har doim bittadan qilinadi (bu Davo
        Ariza bo'limining o'zida amalga oshiriladi).
        """
        selected = self._effective_selection()
        if not selected:
            messagebox.showinfo("Diqqat", "Kamida bitta mijozni tanlang (katakchani belgilang "
        ...
        if len(selected) > 1:
            ...
            for iid in selected:
                ...
                if r['chora'] in ('xat_yuborish', 'xat_yuborish_keyingi_bosqich'):
                    xat_nomzodlar.append({'portfel': r['portfel'], 'xat': r['xat']})
                elif r['chora'] == 'davo_ariza_tayyorlash' and r['xat']:
                    davo_nomzodlar.append({'portfel': r['portfel'], 'xat': r['xat']})
                else:
                    boshqa += 1

            if xat_nomzodlar and davo_nomzodlar:
                messagebox.showinfo(
                    "Diqqat",
                    "Tanlanganlar orasida ham xat, ham Davo ariza bosqichidagilar bor.\n\n"
                    "Iltimos, birinchi navbatda faqat BITTA turdagi bosqichni (yoki faqat "
                    "xat kerak, yoki faqat Davo ariza kerak bo'lganlarni) filtrlab tanlang."
                )
                return
            if davo_nomzodlar:
                bajar_ommaviy_davo_ariza(self, self.app, davo_nomzodlar)
                self.refresh()
                return
            bajar_ommaviy_xat(self, self.app, xat_nomzodlar, boshqa_bosqich_soni=boshqa)
            self.refresh()
            return
